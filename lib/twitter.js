...
async function pollMentionsAndReply() {
  ...
  try {
    ...
    const mentions = await twitterClient.v2.userMentionTimeline('me', mentionsQuery);
    ...
    claude.logError('Error in Twitter polling:', error);
  }
}
...
function startTwitterPolling() {
  ...
  console.log('🐦 Twitter mention polling active (every 3 minutes)');
  
  // Initial poll
  setTimeout(pollMentionsAndReply, 5000); // Wait 5 seconds after startup
  
  // Set up interval (3 minutes)
  const pollInterval = setInterval(pollMentionsAndReply, 3 * 60 * 1000);
