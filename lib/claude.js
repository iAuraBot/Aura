...
Keep replies short and punchy, like Twitch chat.

Random memes, ironic roasting, hype phrases allowed.

Never break character or sound formal.`;
...
async function getBrainrotReply(userId, userMessage, platform = 'telegram', chatId = null) {
  try {
    ...
    const response = await claude.messages.create({
      ...
      max_tokens: 150, // Keep replies short and punchy
...
// Utility function to check if message should trigger Claude
function shouldTriggerClaude(message, platform = 'telegram') {
  ...
  // Trigger conditions
  const triggers = [
    'chat', 'talk', 'say something', 'respond', 'reply',
    'what do you think', 'opinion', 'thoughts',
    'claude', 'ai', 'bot', 'aura'
  ];
  
  // Check for trigger words or if message is a question
  return triggers.some(trigger => cleanMessage.includes(trigger)) || 
         cleanMessage.includes('?') || 
         cleanMessage.length > 10; // Longer messages more likely to want a response
}
