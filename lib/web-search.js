...
class WebSearchIntegration {
  ...
  async enhanceMessage(userMessage, userId = 'anonymous', platform = 'telegram') {
    ...
    // Smart crypto detection - fetch the right coin based on user query
    if (/bitcoin|btc|crypto|ethereum|eth|solana|sol|market|price|stock|trading|investment/gi.test(userMessage)) {
      let cryptoSymbol = 'bitcoin'; // default
      
      // Detect specific cryptocurrencies
      if (/solana|sol\b/gi.test(userMessage)) {
        cryptoSymbol = 'solana';
      } else if (/ethereum|eth\b/gi.test(userMessage)) {
        cryptoSymbol = 'ethereum';
      } else if (/bitcoin|btc/gi.test(userMessage)) {
        cryptoSymbol = 'bitcoin';
      }
      
      cryptoData = await this.getCryptoPrice(cryptoSymbol);
      if (cryptoData) {
