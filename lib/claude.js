...
function sanitizeOutput(output) {
  ...
  // Length cap - prevent spam/exfiltration (allow complete thoughts)
  if (cleanOutput.length > 350) {
    // Find last complete sentence within limit
    let cutoff = cleanOutput.lastIndexOf('.', 347);
    if (cutoff === -1) cutoff = cleanOutput.lastIndexOf('!', 347);
    if (cutoff === -1) cutoff = cleanOutput.lastIndexOf('?', 347);
    if (cutoff === -1) cutoff = 347; // Fallback to hard cutoff
...
async function getBrainrotReply(userId, userMessage, platform = 'telegram', chatId = null, familyFriendly = false) {
  try {
    ...
    const response = await claude.messages.create({
      ...
      max_tokens: 150, // Short but complete sentences for chat style responses
